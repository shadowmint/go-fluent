...
func Test_sql_factory_can_process_insert_statement(T *testing.T) {
  ...
  a.Equals(sql, "INSERT INTO HelloTable (`date`, `key`, `value`) VALUES (?, ?, ?)", "Invalid SQL")
...
func Test_sql_factory_can_process_select_statement(T *testing.T) {
  ...
  a.Equals(sql, "SELECT `date`, `key` FROM HelloTable WHERE date = ? AND key = ? OFFSET 2 LIMIT 4", "Invalid SQL")
  
  stmt.Limit = -1
  stmt.Offset = -1
  sql, _ = i.Select(&stmt)
  a.Equals(sql, "SELECT `date`, `key` FROM HelloTable WHERE date = ? AND key = ?", "Invalid SQL")

  stmt.Limit = 1
  stmt.Offset = -1
  sql, _ = i.Select(&stmt)
  a.Equals(sql, "SELECT `date`, `key` FROM HelloTable WHERE date = ? AND key = ? LIMIT 1", "Invalid SQL")

  stmt.Limit = -1
  stmt.Offset = 1
  sql, _ = i.Select(&stmt)
  a.Equals(sql, "SELECT `date`, `key` FROM HelloTable WHERE date = ? AND key = ? OFFSET 1", "Invalid SQL")
